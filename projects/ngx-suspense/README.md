...
To avoid some part of UI jumping up & down, you can use `<SuspenseList revealOrder="together | forward | backward">` as a parent component to wrap all `<Suspense>`s. Then all `<Suspense>` will resolve at:

- `together`: at same time
- `forward`: according to the DOM order
- `backward`: reverse the DOM order
